...
class FPN(FasterRCNNBase):
    def __init__(self, n_fg_class): #КОНСТРУКТОР
        if opt.backbone == 'vgg16':   # выбор экстрактора
            extractor = load_vgg16_as_fully_convolutional(pretrained=True) # сверточная сеть vgg16 для выделения признаков
        elif opt.backbone == 'resnet101':
            extractor = load_resnet101(pretrained=True) # остаточная CNN для классификации изображения с 101 слоем
        else:
            raise ValueError('Invalid backbone network') # исключение
        ...
        ) # заполняем аргументы базового класса
        normal_init(self.top_layer[0], 0, 0.01) # функция на строке 206
        normal_init(self.top_layer[2], 0, 0.01) # функция на строке 206
        normal_init(self.loc, 0, 0.001) # функция на строке 206
        normal_init(self.score, 0, 0.01) # функция на строке 206

        self.backbone = opt.backbone # сеть которая стоит в конфиге
        self.n_features = opt.n_features # еще один из конфигов

        if self.backbone == 'vgg16':
            # FPN params
            self.extraction_point = [15, 22, 29, 34] # точка извлечения переменная
            self.default_level = 5 # уровень по умолчанию
            # lateral connection
            self.lateral1 = nn.Conv2d(1024, 256, 1, 1, 0) # литералы (кол-во каналов в входном изображении)
            self.lateral2 = nn.Conv2d(512, 256, 1, 1, 0) # литералы (кол-во каналов созданных сверткой)
            self.lateral3 = nn.Conv2d(512, 256, 1, 1, 0) # литералы (размер свертки ядра, шаг свертки)
            self.lateral4 = nn.Conv2d(256, 256, 1, 1, 0) # литералы (отступы к всем четырем сторонам входа)
        else:
            # FPN params
            self.extraction_point = [1, 2, 3, 4] # точка извлечения переменная
            self.default_level = 4 # уровень по умолчанию
            # lateral connection
            self.lateral1 = nn.Conv2d(2048, 256, 1, 1, 0) # литералы
            self.lateral2 = nn.Conv2d(1024, 256, 1, 1, 0) # литералы
            self.lateral3 = nn.Conv2d(512, 256, 1, 1, 0) # литералы
            self.lateral4 = nn.Conv2d(256, 256, 1, 1, 0) # литералы
        normal_init(self.lateral1, 0, 0.01) # функция на строке 206
        normal_init(self.lateral2, 0, 0.01) # функция на строке 206
        normal_init(self.lateral3, 0, 0.01) # функция на строке 206
        normal_init(self.lateral4, 0, 0.01) # функция на строке 206

        # smooth layer
        self.smooth1 = nn.Conv2d(256, 256, 3, 1, 1) # гладкий уровень
        self.smooth2 = nn.Conv2d(256, 256, 3, 1, 1) # гладкий уровень
        self.smooth3 = nn.Conv2d(256, 256, 3, 1, 1) # гладкий уровень
        normal_init(self.smooth1, 0, 0.01) # функция на строке 206
        normal_init(self.smooth2, 0, 0.01) # функция на строке 206
        normal_init(self.smooth3, 0, 0.01) # функция на строке 206
    ...
    def train(self, mode=True):
        ...
        if self.backbone == 'resnet101' and mode:
            ...
            self.extractor[2].train() # указываем что их тестим, для корректной их работы
            ...
            self.extractor[0].apply(set_bn_eval)# ищем и запускаем пакет
            self.extractor[1].apply(set_bn_eval)# BatchNorm
...
def normal_init(layer, mean, std): # это идет из FasterRCNNBase
