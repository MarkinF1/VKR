...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, x, scale=1, gt_bboxes=None, gt_labels=None, original_size=[3000, 4000]):
        if self.training:
            img_size = tuple(x.shape[2:]) # переделываем в кортеж

            # Feature extractor from the base network(e.g. VGG16, ResNet-101)
            feature = self._extract_features(x) # Извлечение признаков из базовой сети
            ...
            with t.no_grad():
                ...
                # clip bbox
                bbox[:, 0::2] = t.clamp(bbox[:, 0::2], min=0, max=original_size[0])
