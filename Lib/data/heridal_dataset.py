...
from Lib.data.utils import read_image
from Lib.data.transforms import TransformsTrain, TransformsTest, TransformsPlot


class HRDLDataset(Dataset):
    def __init__(self, root, split, transform_mode, use_difficult=False):
        super(HRDLDataset, self).__init__()
    ...
    def _load_image_and_target(self, i):
        ...
        for obj in annot.findall('object'):
            ...
            label.append(HRDL_BBOX_LABEL_NAMES.index(name))
    ...
    def test_image_on_object(self, i):
        img_id = self.img_ids[i]
        annot = ET.parse(os.path.join(self.root, 'Annotations', img_id + '.xml'))
...
HRDL_BBOX_LABEL_NAMES = ('nohuman', 'human')
